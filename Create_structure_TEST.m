...
%Office paths
path1 = 'C:\Users\as2307\Dropbox\PhD\Experiment 1 Year 1\Results & Data Analysis\Final Experiment 1 Data\Matlab!!\Final versions\Intrusions Data';
path2 = 'C:\Users\as2307\Dropbox\PhD\Experiment 1 Year 1\Results & Data Analysis\Final Experiment 1 Data\Matlab!!\Final versions\Subjective Data\';
...
%Create structure 'participant' to store all your data
participant = struct();
participant.ID = [];
participant.age = [];
participant.gender = [];
...
load('STIM_Number.mat')

%% Import psychopy data (.xlsx only) and write structure 'participants'

for indx=1:numel(files)
    [~,~,intr_data_raw] = xlsread(fullfile(path1, files(indx).name), 'U32:AN271');
    
    %Import only the columns you want from the raw data file
    ...
    participant(indx).intrusions_data = sortrows(participant(indx).intrusions_data, 1);
    
    participant(indx).intrusions_data(:,9) = mat2cell(Stim_no(:,1));
    
    clear intr_data
    
    participant(indx).titles = unique(participant(indx).intrusions_data(:,1));
    
    %Create indices for your different conditions
    ...
    %Add these trials to participant
...
%% Import Day 1 subjective reports (.xlsx only) to participants
...
%Loop through all participants
    ...
    %previous import from psychopy data file
