...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/akulsatish/Dropbox/PhD/Experiment 1 Year 1/Experimental files/PsychoPy program/Intrusions study PsychoPy program FINAL/ATNT_Intrusions_Phase_FINAL.psyexp',
...
for i in range(num_test):
    ...
    stim_no_item.append(stim_no[stim_order[i]])
    
    print title_item[i] + moral_cond_item[i]
    
    #determine the colour of the items
    if moral_cond_item[i] == "Right":                  #This if statement is to ensure that colour is assigned equally across morality conditions 
        if count_r <=2:
        ...
        if count_w <=2:                                  #All even rows will be green and all odd rows will be red for morally wrong cues
            cue_colour.append("green")
        else:
            cue_colour.append("red")
        count_w = count_w + 1
    ...
    print "item #" + str(i) + ":" + " " + title_item[i]+ " "+ moral_cond_item[i] + " " + cue_colour[i]
...
for thisTNT_loop in TNT_loop:
    ...
    stim_no = stim_no_item[stimulus_number]
    ...
    thisExp.addData('randomised_no', stimulus_number)
    thisExp.addData('stimulus_no', stim_no)
    thisExp.addData('loop_number', loop_number)
